...
NUMTAPES = 2
Div = 120  # Circular division for polar mapping
...
# シリアル通信設定
port = '/dev/ttyUSB0'  # Adjust based on your platform
baudrate = 115200
...
def polarConv(imgOrgin):
	...
	# リサイズ処理
	WIDTH_FULL, HEIGHT_FULL = IMG.size
	if WIDTH_FULL > HEIGHT_FULL:
		RESIZE = IMG.resize((math.floor((NUMPIXELS) / HEIGHT_FULL * WIDTH_FULL), NUMPIXELS+1))
	else:
		RESIZE = IMG.resize((math.floor(NUMPIXELS+1), math.floor((NUMPIXELS) / WIDTH_FULL * HEIGHT_FULL)))
	...
	for x in range(Div):  # For each column of image
		...
		for y in range(int(NUMPIXELS/2)):  # For each pixel in column
			...
			value = PIXELS[img_y % HEIGHT, img_x % WIDTH]  # Read RGB pixel in image
